...
def get_contracts(db: Session, skip: int = 0, limit: int = 50, search: str = None, tab: int = None, shipping_type: str = None, sort_by: str = "modified_date", sort_dir: str = "desc") -> tuple[List[Contract], int]:
    """Get list of contracts with pagination - optimized version with filtering support"""
    from sqlalchemy.orm import selectinload
    ...
    # Search filter - apply at DB level instead of Python
    if search and search.strip():
        search_pattern = f"%{search.strip()}%"
        filters.append(
            or_(
                Contract.contract_no.ilike(search_pattern),
                Contract.seller_contract_ref_no.ilike(search_pattern),
                Contract.vessel_name.ilike(search_pattern)
            )
        )
    
    # Shipping type filter
    if shipping_type and shipping_type != 'all':
        filters.append(Contract.shipping_type == shipping_type)
    
    # Tab/status filter (tab: 0=all, 1=draft, 2=posted, 3=pending_pricing)
    if tab is not None and tab > 0:
        status_map = {1: 'draft', 2: 'posted', 3: 'pricing_pending', 4: 'closed'}
        if tab in status_map:
            filters.append(Contract.status == status_map[tab])
    ...
    # Apply sorting
    sort_column = getattr(Contract, sort_by, Contract.modified_date)
    if sort_dir == "asc":
...
def create_contract(db: Session, contract: ContractCreate, user_id: uuid.UUID) -> Contract:
    ...
    try:
        ...
        if not contract_no:
            ...
            # Frontend provided a contract number - check if it exists
            existing = db.query(Contract).filter(Contract.contract_no == contract_no).first()
            if existing:
                # Generate a new unique number instead of failing
                logger.warning(f"Contract number {contract_no} already exists, generating new number")
                seller_code = "CNT"
                if contract.seller_id:
                    from models.core_models import Seller
                    seller = db.query(Seller).filter(Seller.id == contract.seller_id).first()
...
def update_contract(db: Session, contract_id: uuid.UUID, contract: ContractCreate, user_id: uuid.UUID = None) -> Optional[Contract]:
    ...
    import logging
    logger = logging.getLogger(__name__)
    logger.info(f"Starting update for contract {contract_id}")
    try:
        ...
        logger.info(f"Found contract {contract_id}, starting field update")
        # 1. Update basic fields
        update_data = contract.model_dump(exclude={'items', 'created_at', 'created_by', 'posted_date'})
        
        # Increment version on every update
        if db_contract.version is None:
            db_contract.version = 1
        else:
            db_contract.version = db_contract.version + 1
        
        for key, value in update_data.items():
            setattr(db_contract, key, value)
        
        logger.info(f"Fields updated, checking status logic")
        ...
        logger.info(f"Starting items update")
        ...
        if contract.items is not None:
            if len(contract.items) == 0 and db_contract.status == "draft":
                logger.info("Skipping item synchronization for empty draft payload to preserve existing items")
            else:
        ...
        db.commit()
        logger.info(f"Database commit successful, refreshing contract")
        db.refresh(db_contract)
        logger.info(f"Update complete for contract {contract_id}")
        return db_contract
